...
class Coach:
	...
	def trainEpoch(self):
		...
		epTBLoss_image, epTBLoss_text = 0, 0
		if args.data == 'tiktok':
			epTBLoss_audio = 0
		...
		for i, batch in enumerate(gflownetLoader):
			...
			context = uEmbeds[batch_index]

			self.gfn_opt_image.zero_grad()
			self.gfn_opt_text.zero_grad()
			if args.data == 'tiktok':
				self.gfn_opt_audio.zero_grad()

			tb_loss_image, _ = self.gfn_image.training_losses(context, image_feats, batch_item)
			tb_loss_text, _ = self.gfn_text.training_losses(context, text_feats, batch_item)
			if args.data == 'tiktok':
				tb_loss_audio, _ = self.gfn_audio.training_losses(context, audio_feats, batch_item)

			epTBLoss_image += tb_loss_image.item()
			epTBLoss_text += tb_loss_text.item()
			if args.data == 'tiktok':
				epTBLoss_audio += tb_loss_audio.item()

			if args.data == 'tiktok':
				loss = tb_loss_image + tb_loss_text + tb_loss_audio
			else:
				loss = tb_loss_image + tb_loss_text

			loss.backward()

			self.gfn_opt_image.step()
			self.gfn_opt_text.step()
			if args.data == 'tiktok':
		...
		# Phase 2: Rebuild UI matrix using GFlowNet
		with torch.no_grad():
			...
			for _, batch in enumerate(gflownetLoader):
				...
				# image
				x0_image = self.gfn_image.sample(context)
				scores_image = torch.mm(x0_image, image_feats.t())
				top_item, indices_ = torch.topk(scores_image, k=args.rebuild_k)
				...
				# text
				x0_text = self.gfn_text.sample(context)
				scores_text = torch.mm(x0_text, text_feats.t())
				top_item, indices_ = torch.topk(scores_text, k=args.rebuild_k)
				...
				if args.data == 'tiktok':
					# audio
					x0_audio = self.gfn_audio.sample(context)
					scores_audio = torch.mm(x0_audio, audio_feats.t())
					top_item, indices_ = torch.topk(scores_audio, k=args.rebuild_k)
		...
		# Phase 3: BPR + Contrastive Learning training
